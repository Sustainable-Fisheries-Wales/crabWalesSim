#' update the operating model for brown crab in Welsh waters 
#' 
...
update_om <- function(fore_om=fore_om, 
  ...
  start_om$jitter_fraction <- 0 # turn off jitterring in initial values
  start_om$retro_yr <- 0
  ...
  recdevs <- stats::setNames(ctl_om$SR_parms$INIT[3] * recdevs - ctl_om$SR_parms$INIT[3]^2/2, # bias correction/adjust
  ...
  # update F values for future F scenarios for fleets:3,4,5,7,8 -> historical values are estimated via model-fitting & unchanged
  ctl_om$F_setup2$Fvalue[ctl_om$F_setup2$year>=inityr_fore] <- ctl_om$F_setup2$Fvalue[ctl_om$F_setup2$year>=inityr_fore] * F_multi
